...
本文件就是 W0 Intake 的核心入口，也是 `docs/W0-intake/` 下唯一需要默认先读的规范。它负责判断一件事是否应该占用当前研发注意力，并定义 intake、decision board、roadmap、focus review 的最小治理方式；文件名固定为 `main.md`。
...
适用范围：所有新请求、客户信号、bug、事故复盘、AI 质量问题、成本/安全/合规事项、技术债和维护工作，都先用 W0 判断是否值得占用当前注意力。

最小产出：一个 `planning/work-intake/<work-id>.json` 记录请求，一个 `planning/decision-board/<period>.json` 给出 now、next、later、parked、killed、expedite 或 maintenance 决策；周期性工作再补 `roadmap` 和 `focus-review`。

人工判断点：是否进入 `now` 或 `expedite`，是否打断当前 focus，是否杀掉或停车仍有客户/合规/安全/收入风险的请求，是否接受无证据高影响工作，是否超过本周期 appetite。

出口：证据不足则进入 W1 或停车；需要定义行为、边界、风险和退出条件则进入 W2；发现只是维护、索引或上下文恢复问题则进入 W9；已经完成、拒绝或延期的请求回写 decision board，不继续推进。

## 场景触发规范

- 产品机会、增长实验、用户问题不清：进入 `docs/W1-discovery/main.md`。
- 需要采集指标、埋点或做实验：进入 `docs/W1-discovery/product-analytics-experiment-standard.md`。
- 工作会影响生产、数据、安全、成本、AI 行为或超过 30 分钟：进入 `docs/W2-openspec-risk/main.md`。
- 支持反馈、投诉、退款、信任请求进入：参考 `docs/W8-learn/customer-support-trust-ops-standard.md`。
- 客户试点、租户交付、真实客户上线进入：参考 `docs/W6-release/customer-pilot-onboarding-launch-standard.md`。
- 安全/隐私事故、漏洞、凭据泄露进入：先走 `docs/W7-operate/security-privacy-incident-vulnerability-standard.md`，事后再回到 W0 复盘。
- AI 质量回归或线上行为漂移进入：先走 `docs/W8-learn/ai-quality-regression-incident-standard.md`，事后再回到 W0 决定下一步。
- 文档、索引、依赖、维护类请求进入：参考 `docs/W9-maintain/knowledge-context-recovery-standard.md` 或 `docs/W9-maintain/maintenance-dependency-debt-standard.md`。

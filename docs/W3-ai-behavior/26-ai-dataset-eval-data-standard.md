# 阶段 26：AI 数据集、评测样本、标注与数据刷新治理规范

## 目标

一人公司做 AI 产品时，最容易被低估的风险不是“没有 eval”，而是 eval 背后的数据没有来历、没有许可、没有隐私边界、没有标注口径、没有刷新记录，最后变成一堆看似能跑、实际不可复用也不可解释的样本。第 26 阶段定义 AI 数据集、评测样本、标注与刷新治理规范，让每个 AI capability 都能回答：样本从哪里来，能不能用，用来评测还是训练，谁标注，怎样复查，什么时候需要人判断。
...
- 通用业务日志、完整生产数据仓库、BI 指标体系；这些由阶段 15 观测性、阶段 10 隐私安全、阶段 23 支持反馈和阶段 25 信任政策覆盖。
...
- 删除、retire、移动 eval case 到训练/微调候选集属于高风险数据动作，走阶段 24 后台运营动作审计。
...
结论：可落地。第 26 阶段把 AI 质量从“感觉变好了”压到“这个版本、这些样本、这个口径、这次复盘”，同时不把一人公司拖进重型数据平台。

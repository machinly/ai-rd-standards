# 阶段 36：AI RAG、知识源、检索与引用治理规范

## 目标

RAG 让 AI 产品能使用外部知识，但一人公司最容易在这里踩坑：把未经授权的资料塞进向量库、跨租户检索、文档过期、引用伪造、检索结果被 prompt injection 污染、删除请求无法从 embedding/index 中落实、低置信答案被包装成确定事实。第 36 阶段定义 AI RAG、知识源、检索与引用治理规范，让每个 RAG capability 能回答：哪些来源可用，如何摄取、分块、嵌入、索引、过滤、引用、评测、删除和降级。
...
- 通用长期记忆和用户偏好；阶段 29 管 memory/context，本阶段只管 RAG 知识源和检索链路。
- 训练数据、fine-tuning dataset、eval dataset 标注治理；阶段 26 管数据集与 eval 数据。
...
- index/reindex/backfill 是阶段 33 async job：要有幂等、重试、dead letter、取消、速率限制和成本预算。
...
- RAG eval 连接阶段 4 prompt/eval、阶段 26 eval data、阶段 27 red-team、阶段 15 telemetry、阶段 35 evidence。
- 高风险自动动作不能只靠 RAG 答案触发；必须走阶段 32 tool runtime 权限和审批。
...
结论：可落地。第 36 阶段把 RAG 从“神秘向量库”压成一条可审查的数据链路：来源准入、摄取、检索、引用、评测、删除和降级都有明确最小控制。

# 阶段 32：AI 工具运行时、外部连接器与沙箱治理规范

## 目标

AI 产品真正危险的时刻，不是模型说错一句话，而是模型输出被拿去调用工具：读内部数据、写数据库、发邮件、改权限、触发退款、执行代码、访问 MCP server 或连接第三方 API。第 32 阶段定义 AI 工具运行时、外部连接器与沙箱治理规范，让每个 AI capability 都能回答：模型能看到哪些工具，谁批准了调用，工具参数从哪里来，哪些动作有副作用，如何 dry-run，如何幂等和回滚，工具输出是否可信，失败时怎样停止而不是重试到失控。
...
- 已由阶段 24 完全覆盖的人工 admin action；如果该动作被 AI 调用或建议，本阶段仍要求 tool runtime gate。
...
- 新工具、新 connector、新写权限、新代码执行能力或工具自治等级提升前，必须跑阶段 4/26/27 相关 eval 或对抗样本。
...
- 风险：工具登记可能变成文档负担。缓解：只登记生产可被 AI 调用的工具；纯内部手工动作继续走阶段 24。
...
结论：可落地。第 32 阶段把“agent 能调用工具”压成一个可审查的运行时契约：模型建议，应用执行，权限最小，副作用可见，失败可停。

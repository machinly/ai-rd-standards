...
本文件是 W3 AI Behavior 的触发型专项规范，不是 W3 主入口。只有当 `docs/W3-ai-behavior/00-main.md` 已经判断需要治理 eval 数据集、样本来源、标注口径、train/eval 分离、质量报告或刷新复盘时，才读取本文件。

如果当前只是判断 AI capability 的目标行为、失败行为、拒绝行为或降级行为，先回到 `docs/W3-ai-behavior/00-main.md`。

...
- 普通 prompt、eval、agent workflow 的基础研发；走阶段 4。
- 数据集来源、标注、许可、刷新和 train/eval 分离的基础治理；走阶段 26。
- 生产模型路由、供应商 fallback、route policy 和 eval gate；走阶段 30。
- RAG source、chunking、retrieval、citation 和 grounding；走阶段 36。
- 线上 AI 质量事故、回滚和事故样例沉淀；走阶段 48。
...
- `dataset_refs` 必须链接阶段 26 的 dataset card / eval set / quality report；不在本文件存训练样本原文。
...
- 真实客户数据、支持工单、生产日志、raw prompt/response、客户文件或受监管数据进入训练候选时，必须有人工 checkpoint 和阶段 40/41/42/26 证据。
...
- 验证报告不等同上线许可；上线仍需 rollout decision 和阶段 30 route/eval gate。
...
- `ship-production` 必须链接阶段 30 route policy/eval gate 和阶段 48 quality rollback。
...
2. 写 `training-data-plan`：确认数据来源、split、泄漏检查、隐私/许可和阶段 26 链接。
...
6. 写 `rollout-decision`：no-go、重跑、shadow、小流量或生产；生产必须连接阶段 30/48。
...
- 不在业务代码硬编码 fine-tuned model id；通过阶段 30 route registry / feature flag / config 推进。
- 所有优化运行必须有 audit ref；涉及真实数据、供应商训练文件、模型删除、生产 route 切换时连接阶段 24 admin ops。
...
- 不把 eval set、red-team answer、benchmark answer 或线上事故样例直接加入训练，除非阶段 26 记录 split 迁移和人工 checkpoint。
...
结论：可落地。它补上阶段 4、26、30、48 之间的训练治理空白：先评估和证明，再训练；先验证和回滚，再上线。

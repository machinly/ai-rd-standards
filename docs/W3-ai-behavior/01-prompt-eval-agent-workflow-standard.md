# AI prompt / eval / agent workflow 研发规范

## W3 触发定位

本文件是 W3 AI Behavior 的触发型专项规范，不是 W3 主入口。只有当 `docs/W3-ai-behavior/00-main.md` 已经判断需要更细地定义 prompt、eval、结构化输出、workflow/agent 升级或最小 AI 质量门禁时，才读取本文件。

如果当前只是判断 AI 行为应该先读哪个文件，或只需要定义好坏样本、安全边界和出口，先回到 `docs/W3-ai-behavior/00-main.md`。
...
不在本专项展开：完整 RAG 平台、向量库选型、finetuning、复杂多 agent 组织、实时语音、多模态产品、成本平台、供应商抽象层。这些需要时单独开 OpenSpec change。

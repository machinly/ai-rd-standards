# 阶段 30：AI 模型、供应商路由与降级治理规范

## 目标

AI 产品的模型选择很容易从“临时改一个 model name”变成不可复盘的行为变化：质量、延迟、成本、拒答风格、工具调用、上下文窗口、数据保留和供应商可用性都会一起变。第 30 阶段定义 AI 模型、供应商路由与降级治理规范，让每个用户可见 AI capability 都能回答：默认走哪条模型路线、为什么选它、什么时候降级、如何评测候选模型、失败时用户看到什么、哪些数据能发给哪个供应商、何时需要人判断。
...
- 阶段 4 的 prompt/eval 细节；本阶段关注模型路线和供应商执行边界。
- 阶段 9 的整体成本预算；本阶段只要求 route 级成本/延迟限制并链接成本工件。
- 阶段 14 的 Feature Flag 平台；本阶段只定义模型路由必须如何被配置和回滚。
...
- route 的 `eval_refs` 和 `safety_refs` 必须指向阶段 4、26、27、28 的相关工件或说明为什么暂不适用。
...
结论：可落地。第 30 阶段把模型选择从“临时调参”变成可评测、可降级、可回滚的产品行为契约。

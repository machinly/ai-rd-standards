...
- 跨会话、需要独立验收，或中断后难以恢复上下文的研发批次，须进入可持续记录状态的规格与计划路径。

<!-- rule-id: PLAN-RISK-ROUTE-NOT-DURATION -->
- 判断是否使用 Standard/High-risk 规格路径时，以风险路径和变更性质为依据；任务时长本身不得单独触发该路径。

<!-- rule-id: PLAN-MINIMAL-KERNEL-PRECEDENCE -->
- 计划规则与本规范的正式分类及项目原则冲突时，以正式规范为准。

<!-- rule-id: PLAN-SINGLE-RECORD-DEFAULT -->
- 一份记录足以支持取舍、跟踪和恢复时，禁止再平行拆出 strategy map、work-intake、decision board、roadmap 与 focus review。
...
- 推进服务端 change 前，须确认权威产品输入与体验设计，并创建或继续对应的 OpenSpec change。

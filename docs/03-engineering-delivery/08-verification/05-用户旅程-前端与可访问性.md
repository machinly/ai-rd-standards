...
- 条件“发生页面视觉或响应式变化时”成立时，必须在一轮页面 review 中覆盖手机竖屏 `mobile-portrait`（390 × 844 CSS px）和桌面横屏 `1080p-landscape`（1920 × 1080 CSS px），两种 viewport 均分别检查 `light` 与 `dark`，共四种组合。尺寸指浏览器内容 viewport 且浏览器缩放为 100%；每种组合须保存独立 screenshot 或对应的 Playwright 视觉证据，并记录实际 viewport 与主题。同一窗口只改缩放比例、复用截图或漏检任一主题不得计为完成。不要求按四种视觉组合重复执行完整 Browser E2E；截图或视觉断言也不能替代从页面入口执行真实业务动作的 Browser E2E。

<!-- rule-id: VERIFY-VISUAL-UX-CONFORMANCE -->
- `visual_ux: required` 的 change 须在干净完整本地环境用 Browser E2E 证明关键旅程，并对照当前人类批准的 `flow.md` 与 wireframes，逐项核对 `mobile-portrait`（390 × 844 CSS px）和 `1080p-landscape`（1920 × 1080 CSS px）各自的 `light` 与 `dark` 四种组合，以及适用的 loading、empty、error、success、keyboard-only、visible focus 和 focus order。任务路径、结构、状态、权限含义、高风险确认、固定 viewport 或主题存在实质差异时，记录差异并返回体验设计重新 review；未完成前不得把该路径标为 accepted。

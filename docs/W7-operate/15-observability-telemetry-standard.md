# 阶段 15：观测性、遥测与 AI Trace 规范

## 目标

SLO 和 runbook 告诉我们“什么时候该响应”，但代码本身必须先发出可用信号。第 15 阶段定义 Go/Kratos/gRPC/sqlc、Vite 前端和 AI workflow 的最小遥测标准，让一人公司在事故、成本异常、AI 行为漂移和用户体验变差时能快速定位，而不是靠回忆和翻日志碰运气。
...
- 阶段 5 SLO、阶段 6 release、阶段 9 cost、阶段 10 security、阶段 11 product metrics 和阶段 12 quality gates 需要的信号。
...
- 运维角度：四个黄金信号、trace correlation 和 dashboard drilldown 能支持阶段 5 incident/runbook。
- 安全隐私角度：默认不采样 prompt/response、secret、PII，采样必须人审。
- 成本角度：高基数、高采样率、AI trace 和长期保留都有 cost_controls 和 checkpoint。

结论：可落地。第 15 阶段把“上线后看不见”的风险前移到研发阶段，和 SLO、release、AI eval、成本规范衔接。

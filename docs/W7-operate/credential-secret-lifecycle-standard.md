# W7 Operate 触发专项：凭据、密钥与服务账号生命周期规范

## W7 触发定位

本文件是 W7 Operate 的触发型专项，不是 W7 主入口。只有当当前工作涉及 API key、service account、webhook secret、TLS/private key、OpenAI/provider key、CI/OIDC identity、rotation、revocation 或 credential exposure 时，才需要读取本文件。

普通 W7 运行入口应先回到 `docs/W7-operate/main.md`，由主入口判断是否触发本专项。

## 目标

一人公司的密钥问题通常不是“不知道不能泄露”，而是：到底有哪些凭据、谁在用、在哪里注入、多久没轮换、泄露后先撤哪个、旧版本是否还活着，没有一个可恢复的事实来源。本专项只解决这个窄问题：让生产凭据、API key、service account、webhook secret、TLS/private key、数据库凭据、OpenAI/project key、CI/OIDC identity 和本地开发凭据都有 inventory、访问边界、轮换计划、轮换证据和泄露复盘。
...
- 小型项目管理：一人公司不维护完整 PAM/KMS/GRC 平台；本专项只保留五个工件，把人的注意力留给生产凭据、真实泄露和无法自动轮换的高风险项。
- OWASP Secrets Management：secrets 需要集中存储、访问控制、审计、轮换、撤销、过期和 incident response；metadata 必须能回答创建、使用、轮换、删除、联系人、用途和类型。
- NIST SP 800-57 Part 1：cryptographic key management 覆盖 key material 的保护、用途、生命周期、backup、compromise、inventory 和 policy；本专项只取一人公司能执行的 key inventory 与 lifecycle 片段。
...
- W2 的整体 security/privacy/supply-chain 基线；本专项只负责 credential lifecycle。
- W4 配置与 Feature Flag 专项的普通配置项、feature flag 和 runtime change；只有敏感凭据及其引用进入本专项。
- W7 安全/隐私事故专项的完整安全事故响应；本专项只定义 credential exposure 的撤销、轮换和预防复盘。
...
- 涉及生产、客户数据、费用、跨租户、供应商通知或无法确认是否被使用时，必须链接 W7 安全/隐私事故专项 artifact。
...
- 发现 prompt injection 诱导模型泄露 secret、工具返回 secret、RAG/source 含 secret 时，按 exposure review 处理，并补 W3 red-team case 或 W3 tool runtime guard。
...
- 是否需要触发 W7 安全/隐私事故专项、客户/供应商通知或律师/取证帮助。
...
- 产品角度：凭据泄露会直接导致费用、数据、可用性和客户信任损失；本专项把止血路径前置。
...
- 安全隐私角度：只记录引用和 redacted evidence，不保存真实值；泄露复盘连接 W7 安全/隐私事故专项。
- 成本角度：OpenAI/API provider key 有 project、usage monitoring、rate/spend guard 和 revocation path，能降低盗用费用风险。

结论：可落地。本专项把“别泄露 secret”变成可检查的 credential lifecycle 闭环。

# W7 Operate 触发专项：基础设施即代码、环境拓扑与云资源治理规范

## W7 触发定位

本文件是 W7 Operate 的触发型专项，不是 W7 主入口。只有当当前工作涉及 IaC、云环境、resource inventory、state、plan/apply、drift、production apply、destroy/decommission 或基础设施 emergency change 时，才需要读取本文件。

普通 W7 运行入口应先回到 `docs/W7-operate/main.md`，由主入口判断是否触发本专项。

## 目标

一人公司的基础设施常常从“控制台点一下”开始，最后变成最难恢复的生产状态：资源不知道谁创建、环境差异靠记忆、Terraform state 含敏感信息、手工 hotfix 没回写代码、preview/staging/production 不一致、销毁资源时误删数据、成本和安全暴露没人看。本专项定义基础设施即代码、环境拓扑与云资源治理规范，让每个生产 target 能回答：有哪些环境，哪些资源由 IaC 管，state 在哪里，谁能 apply，如何预览变更，如何处理手工变更和 drift，如何销毁或恢复，哪些地方必须人工判断。
...
- 已由 W6 发布流水线专项管理的应用发布流水线细节；本专项关注云资源、环境和 state。
- 已由 W4 配置与 Feature Flag 专项管理的应用 runtime config/feature flag 细节；本专项关注基础设施层配置与环境拓扑。
...
- 前端公开环境变量必须与W4 配置与 Feature Flag 专项的 config registry 对齐；不得把 secret 放入 Vite client env。
...
- AI 成本/容量限制连接 W2 成本容量专项；model route 连接 W3 模型路由；RAG 资源连接 W3 RAG；tool runtime 连接 W3 工具运行时。
...
结论：可落地。本专项把云基础设施从“控制台状态”压成可版本化、可计划、可审查、可恢复的最小治理闭环。

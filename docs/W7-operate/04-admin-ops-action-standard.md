# W7 Operate 触发专项：后台运营、人工操作与高风险动作规范

## W7 触发定位

本文件是 W7 Operate 的触发型专项，不是 W7 主入口。只有当当前工作涉及 admin/support console、人工生产动作、one-off command、dry-run、break-glass、审计日志、生产 SQL 或 AI operator action 时，才需要读取本文件。

普通 W7 运行入口应先回到 `docs/W7-operate/00-main.md`，由主入口判断是否触发本专项。

## 目标

一人公司最危险的生产操作，常常不是正式发布，而是“我临时进后台改一下”“跑个 SQL 修一下”“让 AI agent 帮我处理一下”。这些动作绕过 release pipeline、测试、权限和审计，会直接影响钱、权限、数据、隐私、用户信任和恢复能力。本专项定义后台运营、人工操作与高风险动作规范，让每个生产 target 都能回答：哪些动作允许人做，谁能做，何时需要批准，怎么 dry-run，怎么回滚，怎么审计，AI 能帮到哪一步。
...
- 已完全由 migration/backfill 规范覆盖的正式数据迁移；但触发 migration 的人工审批仍属于本专项。
...
- 运维角度：dry-run、rollback、abort、break-glass 和 review 连接 W7 SRE-lite 和 W7 备份恢复专项的恢复能力。
...
结论：可落地。本专项补上了前面各阶段共同需要的“人操作生产”的安全面：不是不让人动，而是让每次动都有边界、证据和后悔药。

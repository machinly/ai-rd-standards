# 阶段 23：客户支持、反馈分流与信任运营规范

## 目标

一人公司上线 AI 产品后，支持消息会同时包含 bug、用法困惑、计费权益、AI 输出投诉、安全隐私请求、事故反馈和流失信号。如果这些都停留在聊天记录里，研发会被动救火，产品学习也会丢失。第 23 阶段定义客户支持、反馈分流与信任运营规范，把支持队列变成可恢复用户、可保护信任、可反哺研发的最小闭环。
...
- 支持反馈可以进入 eval，但必须脱敏、最小化，并连接阶段 4 的 prompt/eval 规范。
...
- 运维角度：incident handoff、状态页和单一事实来源连接阶段 5 的 SRE-lite。
- 安全隐私角度：默认不收 raw prompt、raw response、secret、支付信息或不必要个人数据；高风险请求进入人审。
- 成本角度：减少重复支持需求比堆客服自动化更适合一人公司，也能暴露 docs、UX 和计费权益缺陷。

结论：可落地。第 23 阶段把阶段 11 的产品学习、阶段 4 的 AI eval、阶段 5 的事故沟通、阶段 22 的计费权益和阶段 10 的安全隐私串成一个用户信任闭环。

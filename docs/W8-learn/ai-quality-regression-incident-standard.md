# W8 Learn 触发专项：AI 质量回归、线上质量事故与回滚规范

## W8 触发定位

本文件是 W8 Learn 的触发型专项，不是 W8 主入口。只有当当前工作涉及 AI 质量回归、线上 AI 质量事故、质量信号、回滚 runbook、质量 incident log、quality review 或事故样本回流 eval 时，才需要读取本文件。

普通 W8 学习入口应先回到 `docs/W8-learn/main.md`，由主入口判断是否触发本专项。

## 目标

AI 产品上线后的失败不总是“服务挂了”。更常见的是：回答变差、格式偶发失败、检索引用跑偏、工具调用变多、拒答风格突变、某类用户任务突然不可用、模型供应商小版本变化带来行为漂移。W3 已要求变更前有 eval，W7 要求生产遥测，W3 要求模型路由和 fallback；本专项负责把这些信号连成一条事故闭环：发现质量回归、分级、止血、回滚、补 eval、复盘。
...
- 安全/隐私/漏洞事故的通知、证据保全和法律边界；这些走 W7 安全/隐私事故专项。
- prompt/eval fixture 的基础创建；这些走 W3 prompt/eval 和数据集专项。
- 通用可用性、延迟、容量事故；这些走 W7 SRE-lite、W2 成本容量、W7 观测性和 W3 模型路由专项。
...
5. 再补 eval：把事故样例加入 W3 eval 或 dataset，避免同类问题回归。
...
- 如果事故实际是安全、隐私、漏洞、资金或权限事故，必须升级到 W7 安全/隐私事故专项、W7 后台运营专项或 W9 审计证据专项，而不是只留在质量日志。
...
- 质量事故中的用户提示必须和W4 notification 专项、W8 support 专项、W6 external claim gate 对齐。
...
结论：可落地。本专项把“AI 变差了”从感觉变成一条可执行事故路径，同时保护一人公司的注意力。
...
结论：可落地。它补上了 W3 eval、W7 telemetry、W3 model routing 之间的运行闭环：上线前有 eval，上线中有信号，出问题时有止血和学习。

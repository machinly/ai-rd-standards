...
- 新工作是否进入 now/expedite，回到 `docs/W0-intake/main.md`。
- 用户问题、目标用户、成功指标和产品赌注重新定义，回到 `docs/W1-discovery/main.md`。
- OpenSpec、风险边界、合同、安全、成本或信任承诺变化，回到 `docs/W2-openspec-risk/main.md`。
- AI 行为、prompt/eval、模型路由、RAG、工具权限或记忆策略重新设计，回到 `docs/W3-ai-behavior/main.md`。
- 实现修复、配置、迁移、worker、计费、通知或开发者文档改动，回到 `docs/W4-build/main.md`。
- 发布、客户上线、对外声明或合同承诺，回到 `docs/W6-release/main.md`。
...
- 支持队列、用户反馈、回复模板、反馈账本、事故沟通、退款/权益/隐私安全请求、top contact drivers：`docs/W8-learn/customer-support-trust-ops-standard.md`
- AI 质量回归、线上 AI 质量事故、质量信号、回滚 runbook、质量 incident log、quality review：`docs/W8-learn/ai-quality-regression-incident-standard.md`
...
- 当前 W8 目录只有一个 `main.md` 作为核心入口。
- 所有其它 W8 文件都是触发型专项，并在开头说明不是主入口。
- 入口、索引和 source map 都指向 `docs/W8-learn/main.md` 与语义化专项文件名。

# 阶段 46：对外承诺、声明与证据发布门禁规范

## 目标

一人公司做 AI 产品时，风险常常不是“没有做”，而是“说法跨了界”：官网写了不训练，供应商配置没有核对；文档说稳定 API，契约还在 experimental；销售邮件承诺 24/7，SRE 只定义了工作时间响应；AI disclosure 说有人审，但产品里没有状态机；客服模板把临时 workaround 说成长期能力。阶段 46 的目标是把所有对外承诺、声明和强说法压成一个发布前门禁：每句话都能追到证据、owner、最后验证时间、适用范围和更正路径。

本阶段不替代阶段 25、35、40、41、43、45。它们分别管理信任政策、证据保全、客户数据生命周期、供应商处理、商业义务和开发者文档；阶段 46 负责在发布前确认这些工件对同一个 claim 给出一致答案。
...
- 已经由阶段 25/35/40/41/43/45 管理的底层工件本身；本阶段只做跨 surface 的发布一致性和证据门禁。
...
- `source_of_truth` 必须链接阶段 25、35、40、41、43、45 或代码/配置/eval/观测中的事实源。
...
结论：可落地。阶段 46 把“能不能对外这样说”变成小而硬的发布门禁，保护一人公司的注意力、信任和现金流。

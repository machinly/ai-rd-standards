...
本阶段的目标不是搭一个复杂平台，而是让“我现在能不能发布”这件事从感觉变成可检查事实。
...
不在本阶段展开：云厂商 Terraform 模块、Kubernetes GitOps 平台、复杂多环境审批矩阵、完整供应链合规、企业级发布列车。这些需要真实产品规模或合规要求时再开独立 OpenSpec change。
...
- 与阶段 5 的 `ops/slo` 和 `ops/release` 复用，不新增多人审批流程。

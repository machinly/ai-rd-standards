# 阶段 43：客户合同、订单、SLA 与商业承诺治理规范

## 目标

一人公司做 AI 产品时，最危险的工程风险之一不是“代码不会写”，而是合同、订单、报价、SLA、销售邮件、官网承诺和真实系统能力不一致：答应 24/7 支持但没有值班；签了 99.99% SLA 但没有 SLO；承诺数据驻留但供应商链路跨境；保证 AI 输出准确但没有 eval；给了服务积分但计费系统没有 credit ledger。阶段 43 的目标是把客户合同和商业承诺转成可版本化、可验证、可复盘的工程义务。

本阶段不是法律意见；它只定义研发和运营如何管理“已经或可能被承诺的东西”。正式合同、条款解释、赔偿、法律合规和争议处理仍需要专业审阅。
...
- 阶段 5、22、25、41、42：SRE、计费权益、信任政策、供应商处理方和 IP 来源已经定义了底层证据；阶段 43 负责把这些证据绑定到客户承诺。
...
- `risk_level=high` 的义务必须有 `human_checkpoint`，并链接阶段 5/22/25/41/42 中相应证据。
...
- 服务积分必须能连接阶段 22 的 billing ledger / credit note / entitlement adjustment；不得只写在邮件里。
...
- AI 能力的客户承诺必须连接阶段 4/26/27/30/32/36/41/42 的 prompt、eval、model routing、tool runtime、RAG、vendor 和 IP evidence。
...
默认不问：低风险标准条款、字段命名、无 SLA 的免费/beta 试用、普通月付/年付订单、已在阶段 22/25/41 覆盖的标准说明。
...
- 安全隐私角度：DPA、数据驻留、训练/保留、子处理方和 audit evidence 与阶段 41/25/35 连起来。
- 成本角度：24/7、高 SLA、专属环境、无限审计和服务积分会带来真实现金/时间成本，必须先算承受力。

结论：可落地。阶段 43 把“商业承诺”变成研发输入和上线 gate，而不是客户签完后才发现系统不能履行。

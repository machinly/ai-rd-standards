...
- 工作优先级和是否值得做，回到 `docs/W0-intake/main.md`。
- 用户问题、试点成功指标或学习假设，回到 `docs/W1-discovery/main.md`。
- 契约、安全、供应商、数据、成本、信任政策和客户数据生命周期，回到 `docs/W2-openspec-risk/main.md`。
- AI 行为、eval、红队、模型路由、工具权限和 RAG 证据，回到 `docs/W3-ai-behavior/main.md`。
- 代码、配置、迁移、计费、通知、worker 或集成实现，回到 `docs/W4-build/main.md`。
- 上线前验证、性能、可访问性或韧性证据，回到 `docs/W5-verify/main.md`。
...
- CI/CD、release artifact、smoke、rollback、production deploy、post-deploy watch、provenance/SBOM：`docs/W6-release/release-pipeline-standard.md`
- 客户试点、租户导入、客户上线、launch readiness、success plan、handoff/offboarding：`docs/W6-release/customer-pilot-onboarding-launch-standard.md`
- 官网、docs、pricing、AI disclosure、trust/security/privacy、developer docs、support/sales claim：`docs/W6-release/external-claim-evidence-release-gate-standard.md`
- 客户合同、订单、SLA、服务积分、红线、DPA、安全附件、商业承诺：`docs/W6-release/commercial-contract-obligation-standard.md`

常见跨 W 触发：

- Billing launch 或 entitlement 执行：回到 `docs/W4-build/billing-entitlement-metering-standard.md` 补实现证据。
- Audit evidence package：进入 `docs/W9-maintain/audit-evidence-compliance-standard.md`。
- Trust/data/vendor claim 证据不足：回到 `docs/W2-openspec-risk/main.md`。
...
- 当前 W6 目录只有一个 `main.md` 作为核心入口。
- 所有其它 W6 文件都是触发型专项，并在开头说明不是主入口。
- 入口、索引和 source map 都指向 `docs/W6-release/main.md` 与语义化专项文件名。

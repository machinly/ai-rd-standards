# W4 Build 触发专项：数据与数据库迁移规范 v0.1

## W4 触发定位

本文件是 W4 Build 的触发型专项，不是 W4 主入口。只有当当前实现涉及 schema、query、migration、sqlc 同步、backfill、生产数据修复、备份恢复前置检查或隐私数据处理时，才需要读取本文件。
...
本专项默认技术栈：PostgreSQL + sqlc + Go/Kratos。migration 工具不强绑定；新 Go 服务没有既有选择时，默认使用 `golang-migrate/migrate` 风格的 `*.up.sql` / `*.down.sql` 文件。
...
不在本专项展开：完整数据平台、数据仓库、CDC 管道、多主复制、跨区域一致性、企业数据治理委员会、复杂 PII 分类系统。这些需要真实业务规模时再开独立 OpenSpec change。

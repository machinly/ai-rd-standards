# 阶段 33：AI 异步任务、队列与后台 Worker 治理规范

## 目标

AI 产品一旦进入真实使用，很多工作都不该绑在一次 HTTP/gRPC 请求里：长推理、批量评测、文件处理、导出、索引刷新、RAG 入库、视频/图片生成、后台补偿、webhook 重试、定时同步、批量数据修复、AI agent 多步任务。第 33 阶段定义 AI 异步任务、队列与后台 Worker 治理规范，让每个后台 workflow 都能回答：任务怎样入队，状态怎样变化，谁能取消，worker 怎样抢占和续租，失败怎样重试，何时进入死信，能否安全回放，用户如何查看进度，部署和宕机时怎样不丢任务、不重复副作用、不无限烧钱。
...
- 阶段 4 AI eval、阶段 15 observability、阶段 24 admin ops、阶段 30 model routing、阶段 32 tool runtime 中任何需要异步执行的 workflow。
...
- 对外部副作用使用 outbox、幂等 key、补偿 action 或阶段 32 tool runtime gate；不能让 retry 重复发邮件、重复扣费、重复改权限。
...
结论：可落地。第 33 阶段把后台任务从“代码里悄悄跑”变成可审查的产品/运维契约：能排队、能停止、能重试、能解释、能恢复。

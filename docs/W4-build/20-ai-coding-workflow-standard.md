# 阶段 20：AI 协作编码、变更批次与自审规范

## 目标

一人公司使用 AI 写代码时，真正的风险不是“AI 会不会写”，而是它能不能始终围绕正确的目标、小批量推进、留下可 review 的证据，并在越界时停下来让人判断。第 20 阶段定义 AI 协作编码规范，把 OpenSpec、Codex/AI agent、代码审查、验证命令和人工 checkpoint 连接成一条轻量闭环。
...
- 已由阶段 4 prompt/eval、阶段 6 release、阶段 10 security、阶段 18 contract 覆盖的深层专项工件；本阶段只要求 AI 编码会话层的证据。
...
- prompt、model、tool schema、agent loop、自主权限、fallback 路径变化必须链接阶段 4 eval 工件。
...
结论：可落地。第 20 阶段把“AI 帮我写代码”升级为“AI 在可审查边界内交付小批次变更”，并与阶段 1 OpenSpec、阶段 4 eval、阶段 12 testing、阶段 19 workspace automation 衔接。

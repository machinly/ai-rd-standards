# 阶段 2：Go/Kratos/sqlc/gRPC 服务端研发规范 v0.1
...
不在本阶段展开：前端 Vite、完整 SRE 告警、CI/CD 平台、云部署、认证/多租户、事件总线。这些后续单独开 OpenSpec change。

## 依据转译

- 《人月神话》：代码生成、框架和 AI 都不是银弹。规范要减少偶然复杂度，不能把简单 CRUD 做成分布式系统论文。
- 小型项目管理：只保留能减少返工的计划项。阶段 2 的计划项就是 API、数据、实现、测试、观测五个切面。

# 阶段 14：配置、环境、Feature Flag 与运行时变更规范

## 目标

一人公司常见事故不一定来自代码错误，而是来自配置漂移、环境差异、开关长期遗留、运行时改动无人记录、前端误暴露密钥或 AI 模型参数悄悄变化。第 14 阶段定义最小配置与运行时变更规范，让配置、环境、Feature Flag、kill switch 和运行时变更可审查、可回滚、可清理。
...
- Experiment toggle：连接阶段 11 产品实验，有 success/stop criteria 和 cleanup。
...
- 产品角度：experiment flag 必须连接阶段 11 实验，不让开关替代产品决策。
...
结论：可落地。第 14 阶段补上了运行时行为控制层，和发布、安全、成本、AI eval 规范互相衔接。

# W9 Maintain 触发专项：知识管理、文档与上下文恢复规范

## W9 触发定位

本文件是 W9 Maintain 的触发型专项，不是 W9 主入口。只有当当前工作涉及 docs map、context pack、glossary、how-to、freshness、canonical 文档入口或 Codex handoff 时，才需要读取本文件。

普通 W9 维护入口应先回到 `docs/W9-maintain/00-main.md`，由主入口判断是否触发本专项。

## 目标

一人公司的文档问题不是“没人写”，而是文档散落、入口不清、术语漂移、过期无提示，导致中断几周后难以恢复上下文，AI 助手也只能重新猜。本专项定义最小知识管理规范，让产品意图、系统边界、命令、常见任务、术语和未决问题有稳定入口。
...
```json
{"date":"2026-06-24","target":"ai-assistant","doc":"knowledge/context-packs/<target>.md","change":"reviewed","reason":"stage 16 baseline","result":"fresh","reviewer":"solo-founder"}
```
...
结论：可落地。本专项补上“研发知识如何被下一次行动使用”的闭环。

# W9 Maintain 触发专项：AI 产品审计、证据保全与合规证据包治理规范

## W9 触发定位

本文件是 W9 Maintain 的触发型专项，不是 W9 主入口。只有当当前工作涉及 audit evidence、audit log、retention、evidence package、customer/auditor evidence、SOC 2 readiness 或对外证据导出时，才需要读取本文件。

普通 W9 维护入口应先回到 `docs/W9-maintain/00-main.md`，由主入口判断是否触发本专项。

## 目标

一人公司的信任问题常常不是“没有做事”，而是半年后说不清当时做了什么、为什么这么做、谁批准、证据在哪里、能不能给客户或审计方看。本专项定义 AI 产品审计、证据保全与合规证据包治理规范，让每个生产 target 能回答：哪些行为需要审计证据，证据来自哪里，如何防篡改，保留多久，能否对外，怎样脱敏，什么时候必须人工判断。
...
- 原始日志、生产数据库、对象存储、SIEM、trace backend 的完整替代；本专项只定义“如何索引和打包证据”。
...
- 对外承诺证据必须连接 W2 commitment register；发布证据连接 W6 release；事故证据连接 W7 SRE-lite。
...
- 高风险 action 连接 W7 admin action；AI tool side effect 连接 W3 tool runtime；async side effect 连接 W4 async job；webhook replay 连接 W4 webhook/event。
...
- 模型或供应商变更的证据必须连接 W3 model routing；prompt/eval 变更连接 W3 prompt/eval 和数据集；安全红队连接 W3 red-team；内容安全连接 W3 content safety；工具运行时连接 W3 tool runtime。
...
结论：可落地。本专项把“将来怎么证明”前移到研发规范里，同时避免把证据目录变成新的敏感数据仓库。

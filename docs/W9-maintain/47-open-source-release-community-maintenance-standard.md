# 阶段 47：开源发布、社区贡献与维护边界规范

## 目标

一人公司做 AI 产品时，开源看起来像增长和信任杠杆：公开 SDK、示例、模板、MCP server、Vite starter、Go client、CLI、operator、eval harness 或 RAG connector。但公开仓库一旦存在，就会产生隐形承诺：有人会提 issue、发 PR、报告漏洞、要求 roadmap、依赖你的 package、引用你的文档、期待安全修复和兼容性。阶段 47 的目标是把开源发布和社区协作压成一套可维护的最小系统：公开什么、接受什么贡献、如何响应、安全漏洞怎么走、什么时候可以说不、何时暂停或归档。

本阶段不替代阶段 42 的 IP/license/provenance，也不替代阶段 44 的安全事故响应、阶段 45 的开发者文档、阶段 18 的 API 兼容性。它负责公开仓库本身的维护边界和社区入口。
...
- 阶段 42 已覆盖的第三方材料使用许可判断；本阶段只处理你对外发布和接受贡献的维护方式。
...
- 如果仓库是客户依赖的官方 SDK 或工具，release notes 连接阶段 45 developer changelog 和阶段 18 API compatibility。
...
- Protobuf breaking changes 连接阶段 18；SDK/generated client release 连接阶段 45。
...
- 公开 demo 不承诺生产 SLA、数据保留、不训练或安全能力，除非阶段 46 claim gate 已通过。
...
- 开源 AI 示例默认使用 sandbox/local mock/synthetic data，和阶段 45 的 developer examples 对齐。
...
结论：可落地。阶段 47 把开源从“把仓库设成 public”升级为可维护、可贡献、可安全发布的研发 surface。

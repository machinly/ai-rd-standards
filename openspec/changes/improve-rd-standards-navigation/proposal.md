...
- 新增 `docs/02-standard-index.md` 作为 workflow-to-standard map。
- 将规范文件移动到 `docs/Wx-*/00-main.md` 与 `<local-order>-<semantic-name>.md` 触发专项，使主入口和目录内顺序一眼可见。
- 新增 `knowledge/` 工件，让 Codex 和人可以恢复上下文。
- 为导航层新增 OpenSpec 规格。

## 不做什么

- 不新增新的 workflow step 或主题索引。
- 不重写既有 55 份规范正文。
- 不改变已有技术偏好和 skill 行为。
- 不建立复杂文档门户或站点。
...
- W0 Intake：一人公司不维护无限 backlog，只维护当前选择和下一步。
